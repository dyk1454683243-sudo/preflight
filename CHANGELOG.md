...
- Session status `ready_for_review` now clears when a pull request opened today is merged in a different session the same day. `gh pr create` and MCP create responses populate `prNumber` from the tool output URL or number. A create with no parseable number still counts as open. A merge on a later day remains outside the today window.
