...
Setting `highSecurity: true` in the config file (or `NEW_RELIC_AI_HIGH_SECURITY=true`) overrides `recordContent` to `false` regardless of any other setting. It also clips free-form error message content before it reaches NR events.
