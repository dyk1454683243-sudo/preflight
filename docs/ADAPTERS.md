...
**Mechanism:** Native `PreToolUse`/`PostToolUse`/`PostToolUseFailure` hooks, installed by Preflight itself, plus seven separate top-level settings.json hooks keys (not `PostToolUse` payload variants): `StopFailure`, which feeds `ApiFailureTracker` with model-API-call failures; `SessionStart`, which fires on every session but is only actionable for resume-cost when Claude Code reports resume-cost fields (`source: 'resume'`/`'fork'` with a prior response) — those feed `SessionResumeTracker`, surfaced in `nr_observe_get_cost_forecast`'s `resumeContext` (a plain startup still reaches `EngagedTimeTracker.recordSessionStart`, which is a no-op for the metric — opening Claude is not engagement); `SessionEnd`, which closes an open engaged-time span (idle-capped — a window left open for hours does not count as engaged); `InstructionsLoaded`, which feeds `InstructionDriftTracker` the exact moment a CLAUDE.md/`.claude/rules/*.md` file enters context — including session-start eager loads, which have no visible `Read` tool call at all; `PostModelSwitch`, which feeds `ModelUsageTracker` a discrete switch event (deliberate `/model` changes, and persistent automatic changes tagged `source: 'auto'`) — `PreModelSwitch` is intentionally not installed, since Preflight has no reason to block or confirm a switch; and `UserPromptSubmit`/`Stop`, which feed precise turn/task-boundary timestamps into `TurnTracker.finalizeTurnAt()`/`TaskDetector.startTaskIfNone()`/`TaskDetector.markBoundary()` and into `EngagedTimeTracker` (prompt→Stop spans unioned with tool-call bursts). Those turn hooks are corroborating signals for the existing idle-gap heuristics, not replacements (`Stop` doesn't fire on a user interrupt, so the heuristics stay as the fallback). No prompt/response content is captured from either hook, only timestamp and session ID.

`ai.session.engaged_ms` is an **approximation** of time the user/agent was doing work, not Claude Code's OTel `active_time.total`. Hooks cannot see keystrokes or window focus; idle gaps longer than 30s between observed events are treated as idle. Do not treat the two metrics as interchangeable. `ai.session.duration_ms` remains wall clock from MCP-process construction.
