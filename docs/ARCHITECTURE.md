...
```
 ┌──────────────────────────────────────────────────────────────────┐
 │                          Claude Code                              │
 │   Pre / Post / lifecycle hooks            MCP client (stdio)      │
 └───────────┬──────────────────────────────────────┬───────────────┘
             │ spawns process                        │
...
| `HookEventProcessor`   | `src/hooks/event-processor.ts` | Polls buffer every 100 ms. Pairs `PreToolUse` and `PostToolUse` events into `ToolCallRecord` objects, and routes lifecycle hook events (`SessionStart`/`SessionEnd`/`UserPromptSubmit`/`Stop`). |
